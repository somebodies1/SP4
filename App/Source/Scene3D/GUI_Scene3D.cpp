...
bool CGUI_Scene3D::Init(void)
{
	...
	cWinScreen->Init();
	cWinScreen->SetShader("Shader_GUI");
	cCameraEffectsManager->Add("WinScreen", cWinScreen);
	// Add LoseScreen
	CLoseScreen* cLoseScreen = new CLoseScreen();
	cLoseScreen->Init();
	cLoseScreen->SetShader("Shader_GUI");
	cCameraEffectsManager->Add("LoseScreen", cLoseScreen);

	//Add DirtScreen
	CDirtScreen* cDirtScreen = new CDirtScreen();
	cDirtScreen->Init();
	cDirtScreen->SetShader("Shader_GUI");
	cCameraEffectsManager->Add("DirtScreen", cDirtScreen);
...
void CGUI_Scene3D::Update(const double dElapsedTime)
{
	...
	//ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.2f, 0.2f, 0.2f, 1.0f));  // Set a background color
	//ImGui::Begin("ReloadTimer", NULL, inventoryWindowFlags);
	//ImGui::SetWindowPos(ImVec2(cSettings->iWindowWidth * 0.5f, cSettings->iWindowHeight * 0.5f));
	//ImGui::SetWindowSize(ImVec2(200.0f * relativeScale_x, 25.0f * relativeScale_y));
	//ImGui::SetWindowFontScale(1.5f * relativeScale_y);
	//ImGui::TextColored(ImVec4(1, 1, 0, 1), "Reload : %3.2f / %3.2f",
	//	cPlayer3D->GetWeapon()->GetReloadTime(), cPlayer3D->GetWeapon()->GetMaxReloadTime());
	//ImGui::End();
	//ImGui::PopStyleColor();
