...
CPlayer3D::CPlayer3D(void)
	...
	, slideTime(0.0f)
	, sMovement(PLAYER_SMOVEMENT::STAND)
{
...
bool CPlayer3D::Init(void)
{
	...
	lastMovement[0] = PLAYERMOVEMENT::FORWARD;
	lastMovement[1] = PLAYERMOVEMENT::FORWARD;
...
{
	if (sMovement != PLAYER_SMOVEMENT::STAND)
		return;
...
void CPlayer3D::SetSMovement(PLAYER_SMOVEMENT newSMovement)
{
	this->sMovement = newSMovement;
}

CPlayer3D::PLAYER_SMOVEMENT CPlayer3D::GetSMovement(void)
{
	return sMovement;
}

void CPlayer3D::SetSlideTime(float newTime)
{
	slideTime = newTime;
}

float CPlayer3D::GetSlideTime(void)
{
	return slideTime;
}
...
{
	if (sMovement == PLAYER_SMOVEMENT::SLIDE)
		return;
	...
	{
		vec3Position -= vec3Front * velocity;
		lastMovement[0] = PLAYERMOVEMENT::BACKWARD;
	}
		
	if (direction == PLAYERMOVEMENT::LEFT)
	{
		vec3Position -= vec3Right * velocity;
		lastMovement[1] = PLAYERMOVEMENT::LEFT;
	}
...
void CPlayer3D::SlideMovement(const float deltaTime)
{
	cout << "Slide Time: "<< slideTime << endl;
	if (sMovement != PLAYER_SMOVEMENT::STAND && sMovement != PLAYER_SMOVEMENT::SLIDE)
		return;
	...
	{
		slideTime = 0.5f;
		sMovement = PLAYER_SMOVEMENT::SLIDE;

	}
	else
	{
		float velocity = fMovementSpeed * deltaTime;
		if (lastMovement[0] == PLAYERMOVEMENT::FORWARD)
			vec3Position += vec3Front * velocity * 3.f;
		if (lastMovement[0] == PLAYERMOVEMENT::BACKWARD)
			vec3Position -= vec3Front * velocity * 3.f;
		if (lastMovement[1] == PLAYERMOVEMENT::LEFT)
			vec3Position -= vec3Right * velocity * 3.f;
		if (lastMovement[1] == PLAYERMOVEMENT::RIGHT)
			vec3Position += vec3Right * velocity * 3.f;
...
bool CPlayer3D::Update(const double dElapsedTime)
{
	...
	if (cCamera)
	{
		//Depending on player_smovement, change camera position
		switch (sMovement)
		{
		case PLAYER_SMOVEMENT::STAND:
			cCamera->vec3Position = vec3Position + cCamera->vec3Offset;
			break;
		case PLAYER_SMOVEMENT::CROUCH:
			cCamera->vec3Position.x = vec3Position.x + cCamera->vec3Offset.x;
			...
			break;
		case PLAYER_SMOVEMENT::PRONE:
			cCamera->vec3Position.x = vec3Position.x + cCamera->vec3Offset.x;
			cCamera->vec3Position.y = vec3Position.y * 0.9f + cCamera->vec3Offset.y;
			cCamera->vec3Position.z = vec3Position.z + cCamera->vec3Offset.z;
			break;
		case PLAYER_SMOVEMENT::SLIDE:
			cCamera->vec3Position.x = vec3Position.x + cCamera->vec3Offset.x;
			cCamera->vec3Position.y = vec3Position.y * 0.95f + cCamera->vec3Offset.y;
			cCamera->vec3Position.z = vec3Position.z + cCamera->vec3Offset.z;
			break;
